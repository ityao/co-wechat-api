...
exports.getwxacode = async function (path, width, auto_color, line_color) {
    const { accessToken } = await this.ensureAccessToken();
    var apiUrl = this.wxaPrefix + 'getwxacode?access_token=' + accessToken;
    var data = {path, width, auto_color, line_color};
    return this.request(apiUrl, postJSON(data));
};

//https://mp.weixin.qq.com/debug/wxadoc/dev/api/qrcode.html
exports.getwxacodeunlimit = async function (scene, page, width, auto_color, line_color) {
    const { accessToken } = await this.ensureAccessToken();
    var apiUrl = this.wxaPrefix + 'getwxacodeunlimit?access_token=' + accessToken;
    var data = {scene, page, width, auto_color, line_color};
    return this.request(apiUrl, postJSON(data));
};